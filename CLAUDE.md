...
**Current state (as of 2026-07-12):** extraction matched 745 UK trip photos + 11 DC/Rania photos (756 total, after resolving the untracked `photos_raw/iphone` deletion slip — 4 more files marked `deleted=true`). Of those, **706 UK trip photos** are live on the site (734 within the 9-day window, minus 28 tagged `final_location = "Ignore"`) — all 9 days have real photo coverage. The user's manual `final_location` curation pass ([#38](https://github.com/harveystephenson/London-2026/issues/38)) is done, and the site was reconciled against it in #49: **571/706 photos group by specific location across 46 map pins** (up from 408/734 across 22 pins pre-curation). Photo links and map pins open a per-event grid popup scoped to that specific location, falling back to the whole day's photos where no match exists yet. Still pending: the `pruned_review/` reconciliation ([#24](https://github.com/harveystephenson/London-2026/issues/24)), the Cotswolds villages (only 35/133 auto-matched — see section 6), and general flag-crowding cleanup now that there are 46 pins ([#50](https://github.com/harveystephenson/London-2026/issues/50)).
...
- **Cotswolds villages only partially resolved (2026-07-12).** The 8 tour-guide stops (Broad Campden, Chipping Campden, Broadway Tower, Broadway, Snowshill, Upper Slaughter, Lower Slaughter, Bourton-on-the-Water) are all in `data/flag_locations.csv` now, but nearest-flag matching only confidently placed 35/133 generically-tagged "Cotswolds" photos (mostly Broadway) — GPS during a driving tour doesn't cluster tightly around village centers the way a point-of-interest visit does. The other 98 remain generically tagged; `final_location` still just says "Cotswolds" for them (untouched — only `suggested_location` was written). Revisit if the user wants to manually assign the rest, or if a looser match tolerance specifically for this day is worth trying.
...
- **Map reshaping / main-page photo placement discussion — in progress, not concluded.** #49 (merged 2026-07-12) added 23 new flags and fixed grouping, driven by the user's actual `final_location` data. Still open: whether every flag needs its own always-visible pin (46 pins now, up from 22 — getting crowded), and where photos surface on the main page (see #47, mockups already shared).
