...
- **Cotswolds sub-location breakdown (received 2026-07-12, not actioned yet).** The user labeled all Cotswolds photos' `final_location` generically as "cotswolds" for now, and shared the tour-guide's actual stop list to break that down further: Broad Campden, Chipping Campden, Broadway Tower, Broadway, Snowhill (likely "Snowshill" — confirm before using), Upper and Lower Slaughter (two villages, may need to split), Bourton-on-the-Water, Burford. Fulfills [#39](https://github.com/harveystephenson/London-2026/issues/39). When the user says their `final_location` spreadsheet review is finished: look up coordinates for each named stop, add them to `data/flag_locations.csv` under `day-03-jul`, then for photos currently labeled "cotswolds" use lat/lon proximity to suggest which specific stop each belongs to — write suggestions somewhere the user reviews and confirms (don't silently overwrite their `final_location`). Do not start until the user explicitly says the spreadsheet review is done.
