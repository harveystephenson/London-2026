...
- **Map reshaping / main-page photo placement discussion — in progress, not concluded.** #49 (merged 2026-07-12) added 23 new flags driven by the user's actual `final_location` data; #52 then removed 10 flags that ended up with zero matched photos (empty pins were "distracting," per the user), landing at **36 pins, all populated** as of 2026-07-12. Still open: Piccadilly Circus/Ambassadors Clubhouse pin overlap and general spacing ([#50](https://github.com/harveystephenson/London-2026/issues/50)), and where photos surface on the main page beyond the map (see #47, mockups already shared via a published Artifact — ask the user for the link if picking this back up, or use `Artifact` tool `action: "list"`).
- **Build speed is a known pain point — tracked in [#54](https://github.com/harveystephenson/London-2026/issues/54).** `build_gallery.py` re-processes every photo on every run with no skip-if-unchanged logic (unlike `extract_trip_photos.py`, which has manifest-reuse idempotency). 7+ minutes for ~700 photos, even after accounting for the OneDrive contention already documented in section 9. Worth profiling before assuming which fix matters most.
