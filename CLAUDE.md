...
4. **Day by day** — 9 collapsible `<details>` blocks, one per trip day. Deliberately stripped down from the original pre-trip planning doc for a family/friends retrospective audience: no Cayman Islands time column, no sunrise/sunset/daylight strip, no "Alt: do X instead" suggestions, no CONFIRMED/NOT BOOKED/CHECK NOW status badges or pills, no per-day Confirmations/warning boxes. Kept: time + activity text, photo links, and a "📺 Videos by Others" strip per day (renamed from "Watch Before You Go" — purpose is added context/color, not pre-trip research; user finds this label inelegant, rename tracked in [#31](https://github.com/harveystephenson/London-2026/issues/31)). **Photo links are photo-first as of PR #56 (2026-07-12, explicit user direction: "emphasis on the photos, not the travel guidance"):** the event *name itself* is the link (`.act-link`, dotted gold underline + small 📷 suffix) opening the per-event grid popup scoped to that location — see section 5 — falling back to the whole day's lightbox otherwise; official websites (Kew, Petersham, Ambassadors, Sky Garden, Sketch) are demoted to a small 🔗 icon after the name. Two rows link straight to day-level photos because their ids have no flag (`sketch`, `liverpool-street` — both were dead-end popups before #56). Map pin popups likewise lead with "📷 View N photos" (real per-location count) and list "Jump to this day →" second.
...
- **Holistic review recommendations (2026-07-12) not yet tracked as issues** — raised in chat, user hasn't dispositioned them; don't act unprompted, but these are the ones worth re-suggesting at natural moments:
  ...
  - Merge #8 + #33 into one EXIF-vs-schedule audit; decide #43 (remove times) first since it shrinks #33 to sequence-checking.
